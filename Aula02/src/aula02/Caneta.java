package aula02;


public class Caneta {
    public String modelo;
    public String cor;
    private float ponta;
    protected int carga;
    private boolean tampada;

    public Caneta(String modelo, String cor, float ponta, int carga) {
        this.modelo = modelo;
        this.cor = cor;
        this.ponta = ponta;
        this.carga = carga;
        tampar();
    }

    public String getModelo() {
        return modelo;
    }

    public void setModelo(String modelo) {
        this.modelo = modelo;
    }

    public String getCor() {
        return cor;
    }

    public void setCor(String cor) {
        this.cor = cor;
    }

    public float getPonta() {
        return ponta;
    }

    public void setPonta(float ponta) {
        this.ponta = ponta;
    }

    public int getCarga() {
        return carga;
    }

    public void setCarga(int carga) {
        this.carga = carga;
    }

    public boolean isTampada() {
        return tampada;
    }

    public void setTampada(boolean tampada) {
        this.tampada = tampada;
    }
    
    
    
    
    
    
    
    
    public void status(){
        System.out.println("Tendes ens tu un mudelo: "+this.getModelo());
        System.out.println("Asta caneta possuíras a cor: "+this.getCor());
        System.out.println("Tens uma ponta di tamanho: "+this.getPonta());
        System.out.println("Esta mesma possuis carga du valor de: "+this.getCarga());
        System.out.println("Possuirás em sua forma atual a presença de tampa?: "+this.isTampada());
    }
    
    public void rabiscar(){
        if(this.tampada==true){
            System.out.println("Deves Destampardes a Caneta para rabiscardes!!");
            if(this.carga==0){
            System.out.println("Para rabiscardes asta preciso que necessitardes duma carga");
            }
        }
        else{
            if(this.carga==0){
            System.out.println("Para rabiscardes asta preciso que necessitardes duma carga");
            }else{
            System.out.println("Rabiscardes");
            this.carga-=1;
            }
        }  
    }
    public void tampar(){
        if(this.tampada==true){
            System.out.println("Tua caneta ja estardes tampada abestalhado!");
        }else{
            System.out.println("A butardes a tampa...");
            this.tampada=true;
        }
        
    }
    public void destampar(){
        if(this.tampada==false){
            System.out.println("Tua caneta ja estardes destampada abestalhado!");
        }else{
            System.out.println("A desbutardes a tampa...");
            this.tampada=false;
        }
    }
    
    
    
    
}
